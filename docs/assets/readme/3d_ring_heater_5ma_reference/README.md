...
This bundle is figure `3d-ring-heater-figure-95722af42c11ffd8`, rendered from numerical run `3d-ring-heater-run-2b84559d95c55d5c`. It uses the public
coarse ring recipe, one Gmsh mesh with 12,761 nodes and
71,808 first-order tetrahedra, and the 5 mA
separately declared low-temperature operating point. Native JAX and locked external Elmer independently solve the same discrete
current/Joule/heat problem at 0.229573 V. No displayed temperature
or parity field is obtained by rescaling another operating point.

The JAX peak temperature rise is 18.261 K. Across all
thermal nodes, the maximum direct-solve Elmer-JAX difference is
2.117e-08 K and the temperature-rise
relative L2 difference is 2.307e-10.
...
uv run python examples/readme_3d_ring_heater_reference.py \
  --render-existing docs/assets/readme/3d_ring_heater_5ma_reference \
...
JAX_PLATFORMS=cpu JAX_ENABLE_X64=1 uv run python \
  ...
  --operating-point low_temperature_projection \
