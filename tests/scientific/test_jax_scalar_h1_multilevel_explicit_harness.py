...
PROBE = REPOSITORY_ROOT / "tests" / "collective_scalar_multilevel_explicit_cpu_probe.py"


def test_four_device_cpu_smoke_keeps_multilevel_transfer_at_jit_boundary() -> None:
    ...
    python_path: tuple[str, ...] = (str(REPOSITORY_ROOT), str(REPOSITORY_ROOT / "src"))
    ...
    assert payload["schema_version"].endswith("explicit_cpu_smoke/v1")
    ...
    assert len(payload["hierarchy_sha256"]) == 64
    assert payload["level_dof_counts"] == [63, 15, 3]
    for report in payload["partitioned_transfer_reports"].values():
    ...
    for report in payload["replicated_transfer_reports"].values():
        assert report["partition_spec"] == []
        assert report["addressable_device_count"] == 4
        assert report["global_replica_logical_bytes"] == (4 * report["logical_bytes_per_replica"])
        assert report["replication_intent"] == "bounded multilevel coarse interpolation"
    tolerances = payload["tolerances"]
    for case in payload["cases"].values():
        assert case["converged"] is True
        assert case["breakdown"] is False
        assert case["setup_valid"] is True
        assert case["iterations"] < case["unpreconditioned_iterations"]
        assert case["relative_residual"] <= 2.0e-5
        assert case["solution_relative_difference"] <= tolerances["solution"]
        assert (
            max(
        ...
        assert case["host_precision_relative_difference"] <= tolerances["host_precision"]
